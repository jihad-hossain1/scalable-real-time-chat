...
const useChatStore = create(
  immer((set, get) => ({
    ...
    createConversation: async (participantIds, isGroup = false, groupData = null) => {
      try {
    ...
    updateConversation: async (conversationId, updates) => {
      try {
        const conversation = await chatService.updateConversation(conversationId, updates)
    ...
    loadMessages: async (conversationId, cursor = null) => {
      try {
        ...
        console.log(`📨 Loaded ${response.messages.length} messages for conversation ${conversationId}`)
        ...
        set((draft) => {
          ...
          if (conversationMessages) {
            ...
            for (const [id, message] of conversationMessages) {
              if (message.isOptimistic && message.content === messageData.content) {
