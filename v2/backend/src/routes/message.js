...
router.get("/search", async (req, res, next) => {
  try {
    ...
    // Validate pagination
    const { error: paginationError } = paginationSchema.validate({
      limit,
      offset,
    });
    if (paginationError) {
      return res
        .status(400)
        .json({ error: paginationError.details[0].message });
    }
...
router.put("/mark-read", async (req, res, next) => {
  try {
    ...
    if (messageIds.length > 100) {
      return res
        .status(400)
        .json({ error: "Cannot mark more than 100 messages at once" });
    }
    ...
    for (const messageId of messageIds) {
      ...
      if (error) {
        return res
          .status(400)
          .json({ error: `Invalid messageId format: ${messageId}` });
      }
    }
