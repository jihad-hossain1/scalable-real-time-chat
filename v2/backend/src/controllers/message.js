...
class MessageController {
      ...
      const directConversations = await db
        ...
        .innerJoin(
          ...
          or(
            and(
              eq(messages.senderId, userId),
              eq(users.id, messages.recipientId)
            ),
            and(
              eq(messages.recipientId, userId),
  ...
  async updateMessageStatus(req, res) {
    try {
      ...
      const isValidRecipient =
        message[0].recipientId === userId ||
        (message[0].groupId &&
          (await this.isGroupMember(message[0].groupId, userId)));
  ...
  async createConversation(req, res) {
    const { conversationType, userId, receiverId, groupId } = req.body;
    try {
      ...
      if (findConversation.length > 0) {
        return res.status(200).json(findConversation[0]);
      }
      ...
      const conversation = await db
        ...
        .values({
          ...
          receiverId,
          groupId,
          isGroup: conversationType == "group" ? true : false,
        })
        .returning();

      console.log(
        "🚀 ~ MessageController ~ createConversation ~ conversation:",
        conversation
      );
  ...
  async getConversationMessage(req, res) {
    try {
